...
void ofApp::update(){
    for(int i=0; i<pos.size(); i++)
    {
        ...
        //3. Set time lapse
        //Built-in function: ofGetElapsedTimef
        //what is born? global variables
        float age = ofGetElapsedTimef() - born[i];
        
        //what is color[i].a? Alpha
        //set the value of alpha to the change of age, and map the value range into 0 - 255;
        
         //4. let balls fade away
        color[i].a = ofMap(age, 0, 5, 255, 0);

        //5. when the ball disappear, it returns to where it begins (to control the # of balls avoid too many)
        if(color[i].a < 10)
        {
            //begin() = returns to where it begins,
            // +i start from ith
...
void ofApp::draw(){
    ...
    //add iterating lines to the screen
...
void ofApp::mouseDragged(int x, int y, int button){
    ...
    pos.push_back( ofPoint(x, y) );//why x,y
